...
function createPostModalStore() {
  const { subscribe, set, update } = writable<PostModalState>({
    isOpen: false,
    variant: 'normal',
    post: null,
    user: null,
  ...
  return {
    subscribe,
    openModal: (variant: 'normal' | 'reply' | 'quote', post: Post | null = null, user: User | null = null) => {
      set({ isOpen: true, variant, post, user });
    },
    closeModal: () => {
      set({ isOpen: false, variant: 'normal', post: null, user: null });
    },
    setIsOpen: (isOpen: boolean) => update(state => ({ ...state, isOpen })),
  };
}
